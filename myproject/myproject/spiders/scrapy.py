...
class QuotesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        item = 1
        conn = psycopg2.connect(dbname="store", user="postgres", password="99babama")
        c = conn.cursor()
        c.execute("CREATE TABLE test15 (id serial PRIMARY KEY, title varchar, herf varchar, image varchar, rating varchar, price varchar, location varchar, amenities text);")
        conn.commit()
        for quote in response.css('div.soom'):
            if item < 12:
                ...
                if img is not None:
                    c.execute("INSERT INTO test15 (title, herf, image, rating, price, location, amenities) VALUES (%s, %s, %s, %s, %s, %s, %s)", (title, herf, img, rating, price, location, amenities))
                    conn.commit()
                elif img is None:
